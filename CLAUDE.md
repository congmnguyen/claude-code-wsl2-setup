...
**claude-notify async (WSL2)**: For Claude Code, wrap the `Stop` hook command as `bash -c '... &'` because the PowerShell script stays alive while the balloon is visible. For Codex, set the top-level `notify` key in `~/.codex/config.toml` (NOT `[tui].notifications`, which is only the in-terminal escape-code notification and runs no external program): `notify = ["bash", "-lc", "msg=$(printf '%s' \"$1\" | jq -r '.\"last-assistant-message\" // \"Done!\"' 2>/dev/null | head -c 120); ~/bin/claude-notify \"Codex\" \"${msg:-Done!}\" &", "--"]`. Codex passes the `agent-turn-complete` JSON as the final argument (`$1`; the `"--"` is `$0`); `jq` pulls `last-assistant-message` so the balloon shows Codex's real reply, falling back to "Done!". Requires `jq`.
